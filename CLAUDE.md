...
2. Run `python -m wif_ag_tool refresh` (re-parses `StrategicDecks.ndf` and recounts vanilla pack indices).
3. Run `python -m wif_ag_tool export` (regenerates all WIF NDF additions with fresh, accurate indices).
4. Run `GenerateMod.bat` (rebuilds the mod `.ndfbin`).
